...
- **Step-by-Step Approach**: 코드를 수정하기 전, 반드시 다음 두 파일을 /docs에 생성/업데이트하여 제시한다.
  1. **[Implementation Plan]**: 구체적인 수정 범위와 로직을 한국어로 기술한 `/docs/implementation-plan-<task-name>.md` 문서.
  2. **[Task]**: 체크박스(`- [ ]`) 형태의 세부 작업 리스트를 작성한 `/docs/task-<task-name>.md` 문서.
...
- **Project References**: 프로젝트 맥락은 `docs/project-context.md`, 구조는 `docs/architecture.md`에서 현재 요청과 관련된 부분만 확인한다.
